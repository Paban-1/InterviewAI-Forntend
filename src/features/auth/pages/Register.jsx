...
import { Mail, Lock, User, Eye, EyeOff, Sun, Moon, ArrowRight, Check } from 'lucide-react';
...
const Register = () => {
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-md bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl rounded-3xl shadow-2xl dark:shadow-indigo-950/20 border border-slate-200/60 dark:border-slate-800/80 p-8 sm:p-10 space-y-8 transition-all duration-300 my-8">
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <div className="space-y-2">
            <label htmlFor="fullName" className="block text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-400">
              User Name
            </label>
            <div className="relative flex items-center">
              <User className="absolute left-3.5 w-5 h-5 text-slate-400 dark:text-slate-500 pointer-events-none" />
              <input
                type="text"
                id="username"
                name="username"
          ...
          <div className="flex items-start gap-3 pt-1">
            <button
              ...
              className={`flex items-center justify-center w-5 h-5 rounded-md border transition-all mt-0.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 ${agreedToTerms
                ? 'bg-indigo-600 border-indigo-600 text-white'
                : 'border-slate-300 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-950/50'
                }`}
            >
              {agreedToTerms && <Check className="w-3.5 h-3.5 stroke-[3]" />}
            </button>
            <label className="text-xs text-slate-500 dark:text-slate-400 select-none leading-normal">
              I agree to the{' '}
